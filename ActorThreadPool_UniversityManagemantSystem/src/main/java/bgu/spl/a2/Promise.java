...
public class Promise<T>{
	 ...
	 */
	public synchronized void resolve(T value){//synchronized in order to avoid 2 threads resolve same promise
		if(isResolved())
	 ...
	 */
	public synchronized void subscribe(callback callback) {//synchronized in order to make sure that thread can't subscribe at the same time the object gets resolved by another thread.
		if(callback!=null) {
			if(!isResolved())
